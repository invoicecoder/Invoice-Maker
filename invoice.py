...
def delete_invoice(invoice_id):
    ...
    if not invoice:
        return redirect(url_for('admin_invoices' if user.is_admin else 'invoices'))
...
def invoices():
    ...
    invoices = query.all()
    invoices_with_remaining = []
    for inv in invoices:
        payment_total = sum(p.amount for p in inv.payments) if inv.payments else 0
        remaining = inv.total - payment_total
        invoices_with_remaining.append({
            "invoice": inv,
            "remaining": remaining
        })
